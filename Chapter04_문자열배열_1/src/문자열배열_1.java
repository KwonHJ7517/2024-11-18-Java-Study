 ...
 *        ---------------------- 한글변환 (브라우저는 char => 1byte)
 *   3) String의 디폴트는 null (주소가 없는 상태)
 *      => 모든 클래스의 디폴트는 null => 에러발생 시 NullPointerExeption
 *   4) String 기능 (메소드)
 ...
 *      boolean starstWith(String s)
 *     ***endsWith() : 끝나는 문자열이 같은 경우
 *      boolean endstWith(String s)
 *        contains() : 포함된 문자열
 ...
 *     int indexOf(char c)
 *     int indexOf(String s) indexOf("Java") => J
 *     ***lastIndexOf() : 끝에서부터 찾기
 *     int lastIndexOf(char c)
 *     int lastIndexOf(String s) indexOf("Java") => J
 *        
 *      = 문자열 결합
 *        concat() : 문자열 결합
