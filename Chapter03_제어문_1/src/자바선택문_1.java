 ...
 *           1) 정수 => 메뉴
 *                 no=0
 *                   ㅣ 0 => 처리문장 0,1,2,3,4까지 처리
 *           switch(no) => 0,1,2 ...
 *           {
 *              case 0:
 *                 처리문장 1
 *                 break; => 종료 (break 는 반드시 주는 것은 아니다)
 *                                같은 내용을 실행할때가 있다
 *              case 1:
 *                 처리문장 2
 *                 break;
 ...
 *                 처리문장 3
 *                 break;
 *              default: ==> else와 같다 => 해당번호가 없는 경우에 처리
 *                 처리문장 4         
 *           }
 *           
 *           예)
 *              90~100 => A => 90~100 => 10으로 나눈다면 => case 문장을 최대한 줄여서 사용
 *              case 10:
 *              case 9:
 *                A
 *                break;
 *                
 *              case 10: case 9: ==> 예) if(score==10 || score==9)
 *                  처리문장
 *                  break;  
 *              
 *                                                        
 */
// 다중조건문 동일 => 한개의 csae 에 있는 문장만 수행
