 ...
 *                 데이터형 변수명;
 *             상수)
 *                 final 데이터형 변수명; => 오류발생 => final 데이터형 변수명=10;
 *                 => 배열의 갯수 => 상수
 *                 => 상수 : 데이터형의 최대값,최소값 
 *        2) 메모리 크기 결정
 *           => 기본형 / 참조형
 ...
 *              3. 숫자는 사용이 가능 (단 앞에 사용 불가능)
 *              4. 특수문자 ( _ , $ )
 *                  _ : 임시변수 , 임시 클래스 , 단어가 여러개인 경우 구분자로 사용
 *              5. 공백을 사용할 수 없다
 *              6. 키워드는 사용할 수 없다
 *              7. 약속 사항
 *                 변수,메소드 => 소문자 시작
 ...
 *                       ----   일반 숫자
 *                 int i=10; => 21억 4천
 *                 char c='A' => 한글도 가능(한글자만 사용)
 *                         '' => null 값이 없는 상태
 *                 long 1=10L;
 *                 
 *                 => 대입하는 수가 21억4천을 넘으면 => L대입
 *              2. 실수형
 *                 double d=10.9D; => D는 생략 가능
 *               
 *              3. 논리형
 ...
 *                           => 데이터 크기 확인
 *                                           (묵시적 형변환)                  (명시적 형변환)
 *                              <=DownCasting: 강제변환          =>UpCasting : 자동변환
 *                              byte < char < int < long < double
 *                               단 boolean 은 형변환이 안된다
 *                              
 *                              long 1=10
 *                                     == int => 자동으로 10L
 *                              int a='A'
 ...
 *                                  char+byte=int   
 *                             / : 
 *                                 1) 0으로 나눌 수 없다 => 나누는 값이 0이 아닌지 확인해야함
 *                                                     => 오류발생함
 *                                 2) 정수/정수=정수
 *                                    10/3 = 3 , 5/2 = 2
 *                                    => 실수로 나눈다 (소수점)                    
 ...
 *                                5%-2=1
 *                                -5%-2=-1
 *                         2. 비교연산자 => 모든 결과값이 boolean => 조건문에서 주로 사용(true , false)
 *                            == 같다
 *                            != 같지않다
 *                            < 작다
 *                            > 크다
 ...
 *                           ---------------------------  
 *                           효율적인 연산
 *                           ** && => 앞에 조건이 false일때 뒤에 조건을 처리하지 않는다
 *                           ** || => 앞에 조건이 true일때 뒤에 조건을 처리하지 않는다
 *                           
 *                           int x=10;
 *                           int y=9;
 *                           
 ...
 *          = 단일조건문
 *           1) 형식 => 흐름(동작순서) => 문법적용
 *                 ㅣ 조건문 사용 연산자 => 부정연산자,논리연산자,비교연산자
 *              if(조건)=> 결과값이 true/false 가 나와야함(false 는 건너뛴다)
 *              {
 *                조건문이 true 일때만 수행하는 문장
 *              }
 *              => 상세보기 / 페이징
 *          = 선택조건문
 *           2) 형식
 ...
 *                조건문이 false 면 처리문장
 *              }
 *              => 로그인 처리 / 아이디 중복 체크 / 우편번호 검색 .....
 *              => 찜하기 , 좋아요 , 예약하기 .....
 *          = 다중조건문 ==> 단순하게 하려면 switch~case
 *            프로그램
 *             = 가독성
 *             = 최적화 (실행속도)
 *             = 재사용성 
