 ...
 *              {
 *                 실행이 가능한 소스 코딩 (정상수행)
 *                 => 에러가 발생할 수 있다 => 에러를 사전에 방지하기 위함
 *                 => 에러가 난 경우 스킵한다 (catch 로 이동)
 *                 ex) if => else
 *              }catch(예외 클래스)
 *              {
 *                 1. 에러확인이 가능
 ...
 *              finally => 생략 가능
 *              {
 *                 try ~ catch 와 상관없이 무조건 수행하는 문장
 *                 ----------- 에러(X),에러(O)
 *                 => 서버 닫기 / 오라클 닫기 / 파일 닫기
 *              }
 *              
 *              ** catch => 에러 발생시 처리하는 영역
 *                 ----- 다중 catch 가능
