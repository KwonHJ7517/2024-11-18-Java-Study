	...
	public void actionPerformed(ActionEvent e) {
		...
		else if(e.getSource()==tf)
		{
			try
			{
				...
				{
					JOptionPane.showMessageDialog(this,"중복 수는 사용할 수 없다");
					tf.setText("");
					tf.requestFocus();
					return; // 메소드 종료
				}
				...
				for(int i=0;i<3;i++)
				{
					for(int j=0;j<3;j++)
					{
						if(com[i]==user[i]) // 컴퓨터와 유저의 같은 수가 있나
						{
							if(i==j) // 같은 자리수냐
								s++;
							else
				...
				if(s==3)
				{
					JOptionPane.showConfirmDialog(this, "프로그램을 종료합니다");
					System.exit(0);
				}
				tf.setText("");
				tf.requestFocus();
